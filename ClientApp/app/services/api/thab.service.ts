...
export class ThabService {
    ...
    public getCertificates(sinumber: number, insz: string): Observable<THABCertificate[]> {
        let url = this.urlService.createUrl('thabcertifcates', sinumber.toString());
    ...
    public getRemark(sinumber: number, certificate: THABCertificate): Observable<string> {
        let url = this.urlService.createUrl('thabcertifcates', sinumber.toString(), 'remarks', certificate.referenceDate.toString());
        
        return this.http.get(url, { responseType: "text"});
    }

    public getPayableAmounts(sinumber: number, certificateid: string): Observable<THABPayableAmount[]> {
        let url = this.urlService.createUrl('thabcertifcates', sinumber.toString(), 'payableamounts', certificateid);
        
        return this.http.get<THABPayableAmount[]>(url);
    }

    public getPayments(sinumber: number, certificateid: string): Observable<THABPayment[]> {
        let url = this.urlService.createUrl('thabcertifcates', sinumber.toString(), 'payableamounts', certificateid);
